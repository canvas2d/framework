...
class RotateTo {
    ...
    updateFrame(percent) {
        this.target.spaceComponent.rotation.update(this.delta * percent + this.from)
        return this
    }
    ...
    static create(to, duration, ease, callback) {
        return (cache.length ? cache.pop() : new RotateTo).init(to, duration, ease, callback)
    }
    static createWithFn(fn, duration, ease, callback) {
        return (cache.length ? cache.pop() : new RotateTo).initWithFn(fn, duration, ease, callback)
    }
    static collect(item) {
        cache.push(item)
    }
