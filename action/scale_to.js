...
class ScaleTo {
    ...
    static create(toX, toY, duration, ease, callback) {
        return (cache.length ? cache.pop() : new ScaleTo).init(toX, toY, duration, ease, callback)
    }
    static createWithFn(fn, duration, ease, callback) {
        return (cache.length ? cache.pop() : new ScaleTo).initWithFn(fn, duration, ease, callback)
    }
    static collect(item) {
        cache.push(item)
    }
