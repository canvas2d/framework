...
class FadeBy {
    ...
    updateFrame(percent) {
        this.target.graphicsComponent.alpha = this.to * percent + this.from
        return this
    }
    ...
    static create(to, duration, ease, callback) {
        return (cache.length ? cache.pop() ? new FadeBy).init(to, duration, ease, callback)
    }
    static createWithFn(fn, duration, ease, callback) {
        return (cache.length ? cache.pop() ? new FadeBy).initWithFn(fn, duration, ease, callback)
    }
    static collect(item) {
        cache.push(item)
    }
